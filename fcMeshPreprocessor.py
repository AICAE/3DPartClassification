...
def generateMetadata(input):
    Mesh.open(input)  # stuck here, why?
...
if __name__ == "__main__":
    ...
    output = input_root_path + u"/occProjector/data/mushroom.stl"
    info = convertMesh(inputfile, output)
    bbox = info["bbox"]
    bbox_arg = " --bbox {} {} {} {} {} {}".format(bbox[0], bbox[1], bbox[2], bbox[3], bbox[4], bbox[5])
    cmd = input_root_path + u'/occProjector/build/occProjector  "{}" '.format(inputfile)  + bbox_arg
    print(cmd)
    os.system(cmd)
    print(bbox)
