...
def get_lib_path_on_windows(fc_name):
    ...
    # the code below assuming freecad executable is on command line search path

    if fc_name and which(fc_name):
...
def get_installaton_path_on_windows(fc_name):
    # search Window Registry, tested for FreeCAD 0.18
    import itertools
    from winreg import (
        ConnectRegistry,
        HKEY_LOCAL_MACHINE,
...
def get_install_prefix(cmod_path):
    # return /usr/  instead of /usr/lib, Linux only
